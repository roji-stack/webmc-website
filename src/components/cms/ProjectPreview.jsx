            ...
            <main className="max-w-5xl mx-auto px-6 py-14">
              ...
              {content_blocks && content_blocks.map((block, index) => {
                ...
                switch (block.type) {
                  ...
                  case "pdf_block": {
                    const fileUrl = block?.file ? (getAsset ? getAsset(block.file)?.toString() : block.file) : null;
                    return (
                      <section key={index} className="bg-white rounded-none p-8 border border-[#E8E4F0] mb-8 shadow-[0_1px_4px_rgba(79,38,131,0.02)] flex justify-center">
                        {fileUrl ? (
                          ...
                          <div className="p-4 border border-dashed border-gray-300 rounded-none bg-gray-50 text-gray-500 text-xs font-mono text-center w-full max-w-xl">
                            [PDF Block: {block?.title || "No Title"} - Upload a file to see preview]
                          </div>
                        )}
                      </section>
                    );
